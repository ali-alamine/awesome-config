...
-- {{{ Tags
	...
	for s = 1, screen.count() do
	    ...
	    tags[s] = awful.tag({ "web", "terminals", 3, 4, 5, 6, 7, "files", "multimedia" }, s, layouts[1])
	end
-- }}}
...
-- {{{ Wibox
	...
	bw_timer = timer({ timeout = 1 })
	bw_timer:connect_signal("timeout", function()
		local textToShow = ""
		local fd = io.popen("upower -d | awk '$1=/percentage/{print $2;exit;}'")
		local str = fd:read("*l")
		io.close(fd)
		fd = io.popen("upower -d | awk '/state/{print $2;exit}'")
		local state = fd:read("*l")
		io.close(fd)
		fd = io.popen("upower -d | awk '/time to empty/{print $4; exit;}'")
		local time = fd:read("*l")
		io.close(fd)
		if not time then
			textToShow = str
		else
			textToShow = str .. " remaining:" .. time .. "h"
		end
		if state == "charging" then
			mybatterywidget:set_markup("<span fgcolor='yellow'>" .. textToShow .. "</span>")
		else
			mybatterywidget:set_markup("<span fgcolor='red'>" .. textToShow.. "</span>")
		end
	end)
	bw_timer:start()
...
-- {{{ Key bindings
	globalkeys = awful.util.table.join(
	    ...
	    awful.key({ modkey,           }, "F3", function() os.execute("xbacklight -inc 40") end),
	    awful.key({ modkey,           }, "F2", function() os.execute("xbacklight -dec 40") end),

			-- Control the sounds
			awful.key({ modkey,           }, "F7", function() os.execute("amixer set Master 2%-") end),
			awful.key({ modkey,					  }, "F8", function() os.execute("amixer set Master 2%+") end),
